...
struct gc_ephemeron {
  ...
  _Atomic struct gc_ref key;
  _Atomic struct gc_ref value;
};
...
static int ephemeron_is_dead(struct gc_ephemeron *e) {
  struct gc_ref key = atomic_load_explicit(&e->key, memory_order_acquire);
  return !key.value;
}
...
void gc_ephemeron_mark_dead(struct gc_ephemeron *e) {
  atomic_store_explicit(&e->key, gc_ref(0), memory_order_release);
}
...
struct gc_ref gc_ephemeron_key(struct gc_ephemeron *e) {
  return atomic_load_explicit(&e->key, memory_order_acquire);
}
